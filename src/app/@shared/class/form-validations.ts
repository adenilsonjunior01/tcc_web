...
export class FormValidations {
  static getErrorMessage(fieldName: string, validatorName: string, validatorValue?: any, message?: string): any {
    const config = {
  ...
  static dateValidator(date: FormControl) {
    if (date && date.value && !dayjs(date.value, 'DD/MM/YYYY', true).isValid()) {
      ...
      if (!date.value.match(ptDatePattern)) return { dateValidator: true };
      return null;
    }
    return null;
  }
